...
from src.config import (
    ...
    PLAYER_PATH_IMAGE,
    PLAYER_POSITIONS,
    PLAYER_START_ATTACK_PATH_IMAGE,
    PLAYER_WIDTH,
    RIGHT_TIMBER_POSITION_LOSE,
    SCORE_FONT,
    TEXT_FONT,
    TIMBER_HEIGHT,
    TIMBER_PATH_IMAGE,
...
class GameEngine:
    ...
    def change_game_speed(self):
        ...
        current_score = max(self.score, 0)

        for difficulty_score, difficulty_config in DIFFICULTY_LEVELS.items():
            if current_score >= difficulty_score:
                difficulty = difficulty_config
            else:
                max_score = difficulty_score
                break

        self.spawn_delay = round(
            difficulty.START_SPAWN_DELAY - (difficulty.START_SPAWN_DELAY - difficulty.END_SPAWN_DELAY) * (
                        current_score / max_score))
        self.components_manager.move_step = round(
            difficulty.START_MOVE_STEP + (difficulty.END_MOVE_STEP - difficulty.START_MOVE_STEP) * (
                        current_score / max_score))
