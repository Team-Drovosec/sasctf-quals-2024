...
// Copied from https://github.com/osirislab/CSAW-CTF-2018-Finals/blob/master/pwn/ES1337/server/server.go
// Edited by m4drat for SASCTF 2024

// run as follows: sudo DOCKER_API_VERSION=1.40 go run server.go
...
Steps to interact with the service:
  * You will solve proof of work
  * You will give us your url to connect to
  * We'll run chrome in the container like this:
    chromium-browser-unstable --no-sandbox --headless=old --disable-software-rasterizer --disable-gpu --virtual-time-budget=60000 <your_url>
  * After you can run arbitrary code, just send the flag to your remote listener
...
func startContainer(url string) {
	...
	resp, err := docker.ContainerCreate(ctx, &container.Config{
		Image: "madrat/ubercaged",
		Cmd:   []string{"chromium-browser-unstable", "--no-sandbox", "--headless=old", "--disable-gpu", "--disable-software-rasterizer", "--virtual-time-budget=60000", url},
	}, nil, nil, nil, "")
	if err != nil {
	...
	if err := docker.ContainerRemove(ctx, resp.ID, container.RemoveOptions{}); err != nil {
...
func handleClient(conn net.Conn) {
	...
	if err != nil || !url.IsAbs() {
		client.WriteString("Thats not a valid url :(")
		return
	}
